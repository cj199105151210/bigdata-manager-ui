<template>
  ...
  <div id="organization">
    ...
    <!-------增加子节点弹窗---------->
    <el-dialog :title="userGroupTitle" :visible.sync="dialogAddForm" :modal="false" 
            :close-on-click-modal="false" :before-close="closeDialog" top>
      <el-form :model="addForm" :rules="rules" ref="addForm" :hide-required-asterisk="ast" label-width="150px">
        <el-form-item label="名称" prop="cname">
          <el-input v-model="addForm.cname" placeholder="请输入机构名称" auto-complete="off" maxlength="40"></el-input>
        </el-form-item>
        <el-form-item label="简称" prop="shortName">
          <el-input v-model="addForm.shortName" placeholder="请输入机构简称" auto-complete="off" maxlength="40"></el-input>
        </el-form-item>
        <el-form-item label="编码" prop="code">
          <el-input v-model="addForm.code" placeholder="请输入机构编码" auto-complete="off" maxlength="40"></el-input>
        </el-form-item>
        <el-form-item label="上级机构" prop="parentId">
          <el-select ref="orgSelect" :value="parentOrg" clearable @clear="clearHandle" style="width:100%;">
            <el-option :value="parentOrg" :label="parentOrg">
              <el-tree  
                accordion
                ref="selectTree"
                :data="parentOrgData"
                ...
                @node-click="handleNodeClickSelectTree">
              </el-tree>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="英文名称" prop="name">
          <el-input v-model="addForm.name" placeholder="请输入机构英文名称" auto-complete="off" maxlength="40"></el-input>
        </el-form-item>
        ...
        <el-form-item label="有效" prop="isEffective">
          <el-radio v-model="addForm.isEffective" label="1">是</el-radio>
          <el-radio v-model="addForm.isEffective" label="2">否</el-radio>
        </el-form-item>
        <el-form-item label="是否实体" prop="isEntity">
          <el-radio v-model="addForm.isEntity" label="1">是</el-radio>
          <el-radio v-model="addForm.isEntity" label="2">否</el-radio>
        </el-form-item>
        <el-form-item label="排序" prop="sortNo">
          <el-input v-model="addForm.sortNo" placeholder="请输入排序号" type="number" oninput="if(value.length > 11)value = value.slice(0, 11)" auto-complete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="新增修改标识" hidden>
          <el-input :value ="addFormFlag" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
      ...
      <div id="dialogMain">
        ...
        <div id="dialogRight" style="width:47.3%;">
          ...
          <div style="width:100%;height:93%;">
            <el-table ref="cacheTable" :data="cacheTableData" height="420" border :header-cell-style="{color:'rgba(0,0,0,.85)'}"
    ...
    <el-dialog :modal='false' width="27%" :visible.sync="addPrivatedialogVisible" 
      ...
      <el-form ref="addPrivateForm" :model="addPrivateForm" :rules="addPrivateNodeRules" label-width="120px" :hide-required-asterisk="ast">
        <el-form-item label="个人组名称" prop="cname">
          <el-input v-model="addPrivateForm.cname" auto-complete="off"></el-input>
        </el-form-item>
...
<script>
  ...
  import { getTreeList,getTableList,getAllUserList,getShowForm,getCacheTableList,insertNode,getNodeById,updateNode,deleteNode,
    getUserByTypeXM,adjustRelation,insertPrivateNode,updatePrivateNode,deletePrivateNode } from "@/api/userCenter/organizationManage";
  ...
  export default {
    ...
    data() {  
      ...
      return {
        ...
        userGroupTitle:'',
        rules: {
          cname: [{ required: true, message: '请填写机构名称', trigger: 'blur' }],
          // shortName: [{ required: true, message: '请填写机构简称', trigger: 'blur' }],
          code: [{ required: true, message: '请填写机构编码', trigger: 'blur' }],
          // parentId: [{ required: true, message: '请选择上级机构', trigger: 'blur' }],
          name: [
            { validator: validateName, trigger: 'blur' }
            // { required: true, message: '请输入机构英文名称', trigger: 'blur' }
          ],
          orgType: [{ required: true, message: '请选择机构分类', trigger: 'change' }],
          isEffective: [{ required: true, message: '请选择是否有效', trigger: 'change' }],
          isEntity: [{ required: true, message: '请选择是否实体', trigger: 'change' }],
          sortNo: [{ required: true, validator: validateSortNo, trigger: 'blur' }]
        },
        showForm: {
          cname: '',
          shortName: '',
    ...
    methods:{
      getTreeData(type,isShowCrew){
        getTreeList(type,isShowCrew).then(res => {
          if (isShowCrew == "1") {
            ...
            this.parentOrgData = res.data.data;
            //给form表单赋值
      ...
      insertNode(){
        ...
        this.$nextTick(() => {
          ...
          this.addFormFlag = "add";
          this.parentOrg = sjOrg;
        });
      }, 
            ...
            if (flag == "add") {
              insertNode(this.addForm).then(res => {
                if (res.data.msg == "success") {
                  ...
                  // 刷新树
                  _this.getTreeData("1","1");
                } else if (res.data.msg == "exist_cname") {
                  _this.$message({
                    showClose: true,
                    message: '机构名称重复！',
                    type: 'warning'
                  });
                } else if (res.data.msg == "exist_code") {
                  _this.$message({
                    showClose: true,
                    message: '机构编码重复！',
                    type: 'warning'
              ...
              if (checkNode[0].id == _this.addForm.parentId) {
                _this.$message({
                  message:"不能选择自身为上级机构",
                  type:"warning"
                });
      ...
      updateNode(){
        ...
        if (checkNode.length === 1) {
          //显示弹窗
          this.dialogAddForm=true;
          this.addFormFlag = "update";
          this.userGroupTitle="编辑";
          //请求数据并赋值
          getNodeById(checkNode[0].id).then(res => {
            if (res.data.msg == "success") {
              //表单赋值
              _this.addForm = res.data.data;
              _this.parentOrg = checkNode[0].parentName;
            } 
      ...
      addCrew(){
        ...
        //调用后台方法调整数据
        if (!flag) {
          adjustRelation("0", this.treeType, "1", this.oldOrgId, userId, orgId).then(res => {
            if (res.data.msg === "success") {
              _this.$message({
                message: '已新增该人员',
                type: 'success'
              ...
              // this.getTableData(this.oldOrgId,1,10);
            } else {
              _this.$message.error('出现错误，请重新操作！');
            }
          }).catch(function(error){
      ...
      removeCrew(){
        ...
        //调用后台调整数据
        if (!flag) {
          adjustRelation("1", "1", this.treeType, targetId, this.currentCacheRow.guid, this.oldOrgId).then(res => {
            if (res.data.msg === "success") {
              _this.$message({
                message: '已移除该人员',
                type: 'success'
      ...
      initHandle(){
        if(this.valueId){
          this.parentOrg = this.$refs.selectTree.getNode(this.valueId).data.title     // 初始化显示
          this.$refs.selectTree.setCurrentKey(this.valueId)       // 设置默认选中
          this.defaultExpandedKey = [this.valueId]      // 设置默认展开
        } 
      },
      // 切换选项
      handleNodeClickSelectTree(node){
        this.parentOrg = node[this.defaultProps.label]
