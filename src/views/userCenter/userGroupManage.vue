  ...
  <div id="userGroup">
    ...
    <el-dialog :title="userGroupTitle" :visible.sync="dialogAddForm" :close-on-click-modal="false" :modal="false" :before-close="closeDialog">
      <el-form :model="addForm" :rules="rules" ref="addForm" :hide-required-asterisk="ast" label-width="150px">
        <el-form-item label="用户组名称" prop="cname">
        ...
        <div id="dialogLeft" style="width:42%;">
          ...
          <div v-if="showLeftTable" style="height:100%;width:100%;">
            <el-table ref="leftTable" :data="leftTableData" height="420" border :header-cell-style="{color:'rgba(0,0,0,.85)'}"
              highlight-current-row style="width: 100%;height:100%;" @current-change="handleCurrentLeftRow">
...
<script>
  ...
  import { getTableList,getAllUserList,getShowForm,getCacheTableList,insertNode,getNodeById,updateNode,deleteNode,
    getUserByTypeXM,adjustRelation,insertPrivateNode,updatePrivateNode,deletePrivateNode } from "@/api/userCenter/userGroupManage";
  ...
  export default {
    ...
    data() {     
      return {
        ...
        userGroupTitle:'',
        rules: {
          cname: [{ required: true, message: '请填写用户组名称', trigger: 'blur' }]
        },
        showForm: {
          cname: '',
          shortName: '',
    ...
    methods:{
      ...
      addCrew(){
        ...
        //调用后台方法调整数据
        if (!flag) {
          adjustRelation("0", this.treeType, "2", this.oldOrgId, userId, orgId).then(res => {
            if (res.data.msg === "success") {
              _this.$message({
                message: '已新增该人员',
                type: 'success'
      ...
      removeCrew(){
        ...
        //调用后台调整数据
        if (!flag) {
           adjustRelation("1", "2", this.treeType, targetId, this.currentCacheRow.guid, this.oldOrgId).then(res => {
            if (res.data.msg === "success") {
              _this.$message({
                message: '已移除该人员',
                type: 'success'
